...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'TagCloud'">
        <xsl:call-template name="RenderTagCloud"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- TagCloud design render -->
  ...
  <xsl:template name="AddStyleAttribute" >
    <xsl:variable name="Style">
      ...
      <xsl:text>; </xsl:text>
      <xsl:text>border-style: solid;	border-width: 2px;	vertical-align: middle;</xsl:text>
    </xsl:variable>
